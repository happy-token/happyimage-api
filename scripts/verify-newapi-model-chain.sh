...
HAPPYTOKEN_USER_TOKEN="${HAPPYTOKEN_USER_TOKEN:-}"

command -v curl >/dev/null || { echo "curl is required" >&2; exit 1; }
command -v jq >/dev/null || { echo "jq is required" >&2; exit 1; }

if [[ -z "$HAPPYTOKEN_USER_TOKEN" ]]; then
  echo "HAPPYTOKEN_USER_TOKEN is required" >&2
  exit 1
fi
...
for _ in $(seq 1 30); do
  restored="$(curl -fsS "$API_URL/api/image-tasks?ids=$task_id" -H "Authorization: Bearer $HAPPYTOKEN_USER_TOKEN")"
