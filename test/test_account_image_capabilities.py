...
os.environ.setdefault("HAPPYIMAGE_AUTH_KEY", "test-auth")
...
class AuthServiceTests(unittest.TestCase):
    ...
    def test_create_user_key_with_custom_password(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            service = AuthService(JSONStorageBackend(Path(tmp_dir) / "accounts.json", Path(tmp_dir) / "auth_keys.json"))

            item = service.create_key_with_value(role="user", name="Alice", key="alice-password")

            self.assertEqual(item["role"], "user")
            self.assertEqual(item["name"], "Alice")
            self.assertNotIn("key_hash", item)

            authed = service.authenticate("alice-password")
            self.assertIsNotNone(authed)
            self.assertEqual(authed["id"], item["id"])

            with self.assertRaisesRegex(ValueError, "这个专用密钥已经存在"):
                service.create_key_with_value(role="user", name="Bob", key="alice-password")
    ...
    def test_user_image_quota_can_be_reserved_and_refunded(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            service = AuthService(JSONStorageBackend(Path(tmp_dir) / "accounts.json", Path(tmp_dir) / "auth_keys.json"))
            item, raw_key = service.create_key(role="user", name="Alice")
            updated = service.update_key(item["id"], {"image_quota": 2}, role="user")
            self.assertEqual(updated["image_quota"], 2)

            identity = service.authenticate(raw_key)
            self.assertTrue(service.reserve_image_quota(identity, 1))
            self.assertEqual(identity["image_quota"], 1)

            service.refund_image_quota(identity, 1)
            self.assertEqual(identity["image_quota"], 2)

            self.assertTrue(service.reserve_image_quota(identity, 2))
            with self.assertRaisesRegex(ValueError, "用户图片额度不足"):
