...
def _extract_paged_items(payload: object) -> tuple[list, int]:
    ...
    if isinstance(inner, dict):
        for key in ("items", "data", "list", "accounts", "records"):
            value = inner.get(key)
    ...
    try:
        ...
        while True:
            ...
            for account in data:
                ...
                cred_status = account.get("credentials_status") if isinstance(account.get("credentials_status"), dict) else {}
                ...
                items.append({
                    ...
                    "has_refresh_token": bool(
                        _clean(credentials.get("refresh_token"))
                        or cred_status.get("has_refresh_token")
                    ),
                })
...
def _fetch_access_tokens_batch(server: dict, account_ids: list[str]) -> tuple[list[str], list[dict]]:
    """Use the data-export endpoint to get full credentials (incl. access_token)
    for a batch of account IDs in a single request.

    The admin list/detail endpoints intentionally strip sensitive fields;
    only the data-export endpoint (`/api/v1/admin/accounts/data`) returns
    the raw credentials map.
    """
    base_url = _clean(server.get("base_url"))
    if not base_url or not account_ids:
        return [], []
...
class Sub2APIImportService:
    ...
    def _run_import(self, server_id: str, server: dict, account_ids: list[str]) -> None:
        ...
        tokens, fetch_errors = _fetch_access_tokens_batch(server, account_ids)

        for err in fetch_errors:
            self._append_error(server_id, err["name"], err["error"])
