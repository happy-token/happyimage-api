...
def create_router() -> APIRouter:
    ...
    async def create_user_key(body: UserKeyCreateRequest, authorization: str | None = Header(default=None)):
        ...
        try:
            custom_key = body.key.strip()
            if custom_key:
                item = auth_service.create_key_with_value(
                    role="user",
                    name=body.name,
                    key=custom_key,
        ...
        log_service.add(
            ...
            "管理员创建用户",
            {
                ...
                "operator_name": admin.get("name"),
                "user_id": item.get("id"),
                "user_name": item.get("name"),
                "amount": item.get("image_quota"),
                "after_quota": item.get("image_quota"),
            },
        )
    ...
    async def update_user_key(
        ...
        admin = require_admin(authorization)
        before_item = auth_service.get_key(key_id, role="user")
        updates = {
            key: value
            for key, value in {
                "name": body.name,
        ...
        if before_item is not None:
            before_quota = before_item.get("image_quota")
            after_quota = item.get("image_quota")
            if before_quota != after_quota or before_item.get("enabled") != item.get("enabled") or before_item.get("name") != item.get("name"):
                amount = None
                if isinstance(before_quota, int) and isinstance(after_quota, int):
                    amount = after_quota - before_quota
                action = "admin_update"
                if before_item.get("enabled") != item.get("enabled"):
                    action = "admin_enable" if item.get("enabled") else "admin_disable"
                ...
                log_service.add(
                    ...
                    "管理员更新用户",
                    {
                        ...
                        "amount": amount,
                        "enabled_before": before_item.get("enabled"),
                        "enabled_after": item.get("enabled"),
                    },
                )
        return {"item": item, "items": auth_service.list_keys(role="user")}

    @router.get("/api/auth/user-quota-logs")
    async def list_user_quota_logs(
        user_id: str = "",
        limit: int = 100,
        authorization: str | None = Header(default=None),
    ):
        require_admin(authorization)
        items = log_service.list(type=LOG_TYPE_USER_QUOTA, limit=max(1, min(limit, 500)))
        if user_id.strip():
            items = [item for item in items if str(item.get("detail", {}).get("user_id") or "") == user_id.strip()]
        return {"items": items}
        ...
        if before_item is not None:
            log_service.add(
                ...
                "管理员删除用户",
                {
                    ...
                    "operator_id": admin.get("id"),
                    "operator_name": admin.get("name"),
                    "user_id": before_item.get("id"),
                    "user_name": before_item.get("name"),
                    "before_quota": before_item.get("image_quota"),
                    "after_quota": None,
                    "amount": None,
                    "enabled_before": before_item.get("enabled"),
