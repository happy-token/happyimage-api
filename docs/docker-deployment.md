...
`HAPPYIMAGE_AUTH_KEY`、`HAPPYIMAGE_PROXY`、`HAPPYIMAGE_BASE_URL`、OIDC 和会话相关环境变量会覆盖 `config.json` 中的对应值。推荐把密钥、代理和部署域名放在 `.env`，把功能开关和运行参数放在 `config.json` 或 Web 设置页。
...
| 变量 | 说明 | 默认值 |
|:--|:--|:--|
| `STORAGE_BACKEND` | 存储后端，可选 `json`、`sqlite`、`postgres`、`git` | `json` |
...
| `data/accounts.json`、`data/auth_keys.json`、`data/accounts.db` | 号池账号、用户密钥或 SQLite 数据库 | 敏感，按生产数据迁移 |
| `data/logs.jsonl`、`data/share_drafts.json`、`data/image_tags.json` | 日志、分享草稿、图片标签 | 需要保留后台数据时迁移 |
| `data/image-gallery-seed/` | 内置官方图库种子数据 | 镜像启动会初始化，通常不需要手动迁移 |
...
只迁移图片历史时，复制 `data/images/`、`data/image_thumbnails/`、`data/image_index.json` 和 `data/image_tags.json` 即可；不要覆盖服务器已有的 `accounts.json`、`auth_keys.json` 或数据库，除非你明确要迁移账号池和用户数据。

安全注意：

- `data/` 不是公开资源目录，里面可能包含 OpenAI access token、refresh token、用户访问密钥、邮箱、密码、代理或第三方服务凭据。
- 当前项目默认不对 `data/` 做静态加密。安全性依赖服务器文件权限、磁盘加密、备份加密和仓库访问控制。
- `.gitignore` 默认忽略 `data/*`，只允许 `data/image-gallery-seed/` 入库。不要把 `data/accounts.json`、`data/auth_keys.json`、数据库文件、日志和备份提交到公开仓库。
- 如果用 `STORAGE_BACKEND=git` 保存账号数据，目标 Git 仓库必须是私有仓库，并使用最小权限 token。
